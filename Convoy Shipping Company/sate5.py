...
#  Making string in xml format, then use fromstring method to write the string to xml file
string = '<convoy>'
for x in range(len(database)):
    string += f'\n\t<vehicle>\n\t\t<vehicle_id>{database.vehicle_id[x]}</vehicle_id>' \
              f'\n\t\t<engine_capacity>{database.engine_capacity[x]}</engine_capacity>' \
              f'\n\t\t<fuel_consumption>{database.fuel_consumption[x]}</fuel_consumption>' \
              f'\n\t\t<maximum_load>{database.maximum_load[x]}</maximum_load>' \
              f'\n\t</vehicle>'
string += '\n</convoy>'
...
if len(new_file) <= 1:
    print(f"{len(new_file)} vehicle was saved into {file_name_no_ext}.xml")
else:
    print(f"{len(new_file)} vehicles were saved into {file_name_no_ext}.xml")